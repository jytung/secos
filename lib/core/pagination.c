...
void pagination()
{
    ...
    pg_set_entry(&kernel_pgd[KRN_idx], PG_RW | PG_KRN |PG_P, page_nr(kernel_ptb));
    ...
    pg_set_entry(&user1_shared_ptb[SHM_idx], PG_RW | PG_USR |PG_P, (SHM_idx << 10));

    pg_set_entry(&user1_pgd[KRN_idx], PG_RW | PG_KRN |PG_P, page_nr(user1_kernel_ptb));
    pg_set_entry(&user1_pgd[U1_idx], PG_RW | PG_KRN |PG_P, page_nr(user1_ptb));
    pg_set_entry(&user1_pgd[SHM_idx], PG_RW | PG_KRN | PG_P, page_nr(user1_shared_ptb));
    ...
    pg_set_entry(&user2_shared_ptb[SHM_idx], PG_RW | PG_USR |PG_P , (SHM_idx << 10));

    pg_set_entry(&user2_pgd[KRN_idx], PG_RO | PG_KRN |PG_P, page_nr(user2_kernel_ptb));
    pg_set_entry(&user2_pgd[U1_idx], PG_RO | PG_KRN |PG_P, page_nr(user2_ptb));
    pg_set_entry(&user2_pgd[SHM_idx], PG_RW | PG_KRN |PG_P, page_nr(user2_shared_ptb));

    /********Shared memory (0xC10000)********/

    pg_set_entry(&user1_shared_ptb[pt32_idx(CPT_USER1_ADDR)], PG_USR | PG_RW |PG_P, page_nr(SHM_ADDR));
    pg_set_entry(&user2_shared_ptb[pt32_idx(CPT_USER2_ADDR)], PG_USR | PG_RO |PG_P, page_nr(SHM_ADDR));
